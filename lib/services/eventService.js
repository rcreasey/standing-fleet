...
module.exports = function (keyGenerator) {
    ...
    Event.findOne({key: key}, '-_id ts key fleetKey type data', callback);
	};

	pub.getByfleetKey = function (fleetKey, callback) {
		Event.findOne({fleetKey: fleetKey}, '-_id ts key fleetKey type data', callback);
	};
	...
	pub.getAll = function (callback) {
		Event.find({}, '-_id ts key fleetKey type data', callback);
	};
