module.exports = function (memberService, settings) {
	...
	pub.initialize = function (req, fleetKey, memberKey) {
		req.session.fleetKey = fleetKey;
		...
		console.log("fleet key: " + req.session.fleetKey)
	};

	pub.linkToMember = function (req, member) {
		delete req.session.linked;

		pub.initialize(req, member.fleetKey, member.key);
	...
	pub.checkIfValid = function (req, next) {
		...
		memberService.getByfleetKey(req.session.fleetKey, function (error, members) {
