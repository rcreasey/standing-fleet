			...
			for (var i in allMembers) {
				if ((Date.now() - allMembers[i].ts) > settings.memberTtl) {
					...
					eventService.addAndGet('memberTimedOut',
							{ name: allMembers[i].name, id: allMembers[i].id }, allMembers[i].armadaKey, _.noop);
				}
			}
		});
	};
			...
			for (var i in allHostiles) {
				if ((Date.now() - allHostiles[i].ts) > settings.hostileTtl) {
					...
					eventService.addAndGet('hostileTimedOut',
							{ name: allHostiles[i].name, id: allHostiles[i].id, systemId: allHostiles[i].systemId, systemName: allHostiles[i].systemName}, allHostiles[i].armadaKey, _.noop);
				}
			}
		});
	};
	...
	var cleanArmadas = function (callback) {
		armadaService.getAll(function (error, allArmadas) {
			for (var i in allArmadas) {
				memberService.getByArmadaKey(allArmadas[i].key, function (error, members) {
					if ( ((Date.now() - allArmadas[i].ts) > settings.armadaTtl) && members.length < 1 ) {
						armadaService.removeByKey(allArmadas[i].key, _.noop);
						eventService.removeByArmadaKey(allArmadas[i].key, _.noop);
						scanService.removeByArmadaKey(allArmadas[i].key, _.noop);
						hostileService.removeByArmadaKey(allArmadas[i].key, _.noop);
					}
				})
			}
		});
