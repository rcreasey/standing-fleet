module.exports = function (logger, _) {
	...
	pub.getByfleetKey = function (type, fleetKey) {
		// logger.log('Getting all ' + type + 's belonging to ' + fleetKey + ' from cache...', 2);
	...
	pub.removeByfleetKey = function (type, fleetKey) {
		// logger.log('Removing all ' + type + 's with armada key ' + fleetKey + ' from cache...', 2);
		store[type] = store[type].filter(function (item) {
			return item.fleetKey !== fleetKey;
		});
	};
