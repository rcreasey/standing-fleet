                ...
                async.each(scanData.pilots, function(pilot, callback) {
                  async.waterfall([
                      ...
                      function(pilot, callback){
                        hostileService.updateAndGet(headerParser.parse(req), pilot, req.session.fleetKey, function(hostile) {
                          callback(null, hostile);
                        });
                      }
                  ], function (error, hostile) {
                    if (error) callback({'status': 'Unable to parse status.'})
                  ...
                  eventService.addAndGet('reportHostile', hostiles, req.session.fleetKey, function(error, event) {
                    if (error) return errorResponse.respond(req, res, 'member', 'Error reporting hostile');
                    successResponse.respond(res);
                  });
                });
              }
